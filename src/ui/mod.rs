...
fn render_active_tab(editor: &Editor, term: &mut Terminal, width: usize, height: usize) {
    ...
    render_layout(&tab.layout, term, 0, 0, width, height);
}
...
/// Split variant, we determine the sizes of the upper and lower/left and right splits, and render them.
fn render_layout(
    layout: &Layout,
    term: &mut Terminal,
    x: usize,
    ...
    match layout {
        Layout::Leaf { view } => {
            render_view(view, term, x, y, width, height);
        }
        Layout::Split {
            axis,
            weight,
            ...
            Axis::Horizontal => {
                ...
                let right_width = (width as f32 - (width as f32 * weight)) as usize;
                let right_x = x + left_width;
                render_layout(&children[0], term, x, y, left_width, height);
                render_layout(&children[1], term, right_x, y, right_width, height);
            }
            Axis::Vertical => {
                let upper_height = (height as f32 * weight) as usize;
                let lower_height = (height as f32 - (height as f32 * weight)) as usize;
                let lower_y = y + upper_height;
                render_layout(&children[0], term, x, y, width, upper_height);
                render_layout(&children[1], term, x, lower_y, width, lower_height);
            }
        },
    }
}

fn render_view(view: &View, term: &mut Terminal, x: usize, y: usize, width: usize, height: usize) {
    if height < 1 || width < 1 {
        return;
    }
    ...
    render_view_status_bar(view, term, x, height - 1, width);

    term.draw_visual_box(x, y, view_vb);
}

fn render_view_status_bar(view: &View, term: &mut Terminal, x: usize, y: usize, width: usize) {
    if width < 1 {
        return;
    }
